...
function WorkshopInspection.start(vehicle, farmId, plusDuration)
	...
	if g_currentMission:getMoney(farmId) < cost then
		...
		vehicle.rvbDebugger:info("WorkshopInspection.start()", g_i18n:getText("shop_messageNotEnoughMoneyToBuy"))
		return
	end
	
	local totalDuration = INSPECTION.TIME + plusDuration
